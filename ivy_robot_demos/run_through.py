...
def show_2d_spline_path(
    ...
    ax.scatter(
        ivy.to_numpy(interpolated_coords[..., 0]).tolist(),
        ivy.to_numpy(interpolated_coords[..., 1]).tolist(),
        s=15,
        c=[[0.2, 0.2, 0.8]],
    )
    ax.scatter(
        ivy.to_numpy(anchor_coords[1:4, 0]).tolist(),
    ...
    ax.scatter(
        ivy.to_numpy(anchor_coords[0:1, 0]).tolist(),
        ivy.to_numpy(anchor_coords[0:1, 1]).tolist(),
        s=100,
        c=[[1.0, 0.6, 0.0]],
    )
    ax.scatter(
        ivy.to_numpy(anchor_coords[-1:, 0]).tolist(),
...
def show_full_spline_path(
    ...
    for a_poses, i_poses, col in zip(
        ...
        ax.scatter(
            ivy.to_numpy(i_poses[..., 0]).tolist(),
            ivy.to_numpy(i_poses[..., 1]).tolist(),
            s=15,
            c=[col],
        )
        ax.scatter(
            ivy.to_numpy(a_poses[1:4, 0]).tolist(),
        ...
        ax.scatter(
            ivy.to_numpy(a_poses[0:1, 0]).tolist(),
        ...
        ax.scatter(
            ivy.to_numpy(a_poses[-1:, 0]).tolist(),
...
def main(interactive=True, f=None, fw=None):
    ...
    num_free_anchors = 3
    num_samples = 100
    constant_rot_vec = ivy.array([[0.0, 0.0, 0.0]])
    constant_z = ivy.array([[0.0]])

    # xy positions

    # 1 x 2
    start_xy = ivy.array([[0.0, 0.0]])
    target_xy = ivy.array([[1.0, 1.0]])
    ...
    # num_anchors x 1
    anchor_points = ivy.expand_dims(ivy.linspace(0.0, 1.0, num_anchors), axis=-1)

    # num_samples x 1
    query_points = ivy.expand_dims(ivy.linspace(0.0, 1.0, num_samples), axis=-1)

    # interpolated spline poses

    # num_samples x 6
    interpolated_poses = ivy_robot.sample_spline_path(
    ...
    # 1 x 3
    start_rot_vec = ivy.array([[0.0, 0.0, 0.0]])
    target_rot_vec = ivy.array([[0.0, 0.0, np.pi]])

    # 1 x 3
    anchor1_rot_vec = ivy.array([[0.0, 0.0, np.pi / 4]])
    anchor2_rot_vec = ivy.array([[0.0, 0.0, 2 * np.pi / 4]])
    anchor3_rot_vec = ivy.array([[0.0, 0.0, 3 * np.pi / 4]])
    ...
    class SimpleManipulator(Manipulator):
        ...
        def __init__(self, base_inv_ext_mat=None):
            ...
            d_s = ivy.array([0.0, 0.0])
            alpha_s = ivy.array([0.0, 0.0])
            dh_joint_scales = ivy.ones((2,))
            dh_joint_offsets = ivy.array([-np.pi / 2, 0.0])
            super().__init__(
    ...
    # 1 x 2
    start_joint_angles = ivy.array([[0.0, 0.0]])
