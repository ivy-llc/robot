...
def test_compute_mico_link_matrices(device, fw):
    ...
    assert np.allclose(
        mico.compute_link_matrices(ivy.array(td.joint_angles), 6),
    ...
    assert np.allclose(
        mico.compute_link_matrices(
            ivy.array(np.tile(np.expand_dims(td.joint_angles, 0), (5, 1))), 6
        ),
...
def test_sample_mico_links(device, fw):
    ...
    mico = MicoManipulator()
    assert np.allclose(
        mico.sample_links(ivy.array(td.joint_angles), 6), td.sampled_link, atol=1e-6
    )
    assert np.allclose(
        mico.sample_links(
            ivy.array(np.tile(np.expand_dims(td.joint_angles, 0), (5, 1))), 6
